...
void measure_battery() {
    ...
    int battery_level = M5Cardputer.Power.getBatteryLevel();
    int battery_mv = M5Cardputer.Power.getBatteryVoltage();
    ...
    if (battery_level >= 0) {
      ...
      battery_percent = ((battery_mv - 3300.0f) * 100.0f) / (4150.0f - 3350.0f);
      battery_ready = true;
    } else {
    ...
    m5::Power_Class::is_charging_t charging_state = M5Cardputer.Power.isCharging();
    if (charging_state == m5::Power_Class::is_charging_t::is_charging) {
      battery_state = BATTERY_STATE_CHARGING;
      battery_indeterminate = false;
    } else if (charging_state == m5::Power_Class::is_charging_t::is_discharging) {
  ...
  #elif BOARD_MODEL == BOARD_RNODE_NG_21 || BOARD_MODEL == BOARD_LORA32_V2_1 || BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_HELTEC32_V4 || BOARD_MODEL == BOARD_TDECK || BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_HELTEC_T114 || BOARD_MODEL == BOARD_TECHO
    battery_installed = true;
...
bool init_pmu() {
    ...
    last_pmu_update = millis();
    return true;
  #elif BOARD_MODEL == BOARD_RNODE_NG_21 || BOARD_MODEL == BOARD_LORA32_V2_1 || BOARD_MODEL == BOARD_TDECK || BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_TECHO
