...
void setup() {
  ...
  #elif BOARD_MODEL == BOARD_TDECK
    rs_deck::return_to_launcher_next_boot();
  #endif
  ...
  #if BOARD_MODEL != BOARD_RAK4631 && BOARD_MODEL != BOARD_HELTEC_T114 && BOARD_MODEL != BOARD_TECHO && BOARD_MODEL != BOARD_T3S3 && BOARD_MODEL != BOARD_TBEAM_S_V1 && BOARD_MODEL != BOARD_HELTEC32_V4 && BOARD_MODEL != BOARD_CARDPUTER_ADV && BOARD_MODEL != BOARD_TDECK
  ...
  LoRa->setPins(pin_cs, pin_reset, pin_dio, pin_busy, pin_rxen, pin_txen);
    ...
    #if BOARD_MODEL == BOARD_CARDPUTER_ADV
      // Cardputer Adv is host-controlled and leaves RF idle at boot. Avoid
      // probing the SX1262 before the M5 display/BLE stack is fully running.
    ...
    #if MCU_VARIANT == MCU_ESP32 || MCU_VARIANT == MCU_NRF52
      #if HAS_PMU == true
        if (!pmu_ready) pmu_ready = init_pmu();
      #endif
    #endif
    update_display();
  #endif
...
void sleep_now() {
    ...
    #if PLATFORM == PLATFORM_ESP32
      #if BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_XIAO_S3 || BOARD_MODEL == BOARD_CARDPUTER_ADV
        #if HAS_DISPLAY
          display_intensity = 0;
          update_display(true);
        #endif
