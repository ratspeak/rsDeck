...
void kiss_indicate_disp() {
	...
	serial_write(CMD_DISP_READ);
			...
			for (int i = 0; i < 512; i++) { escaped_serial_write(fb[i]); }
			for (int i = 0; i < 512; i++) { escaped_serial_write(0x00); }
		#else
...
bool eeprom_model_valid() {
  ...
  if (model == MODEL_11 || model == MODEL_12) {
	...
	if (model == MODEL_FF) {
	#elif BOARD_MODEL == BOARD_CARDPUTER_ADV
	if (model == MODEL_EC || model == MODEL_FF || model == MODEL_FE) {
	#elif BOARD_MODEL == BOARD_GENERIC_ESP32
	if (model == MODEL_FF || model == MODEL_FE) {
	#else
	if (false) {
...
// Self-provisioning shared by the Cardputer Adv and T-Deck dual-boot builds.
#if BOARD_MODEL == BOARD_CARDPUTER_ADV || BOARD_MODEL == BOARD_TDECK
  #if BOARD_MODEL == BOARD_TDECK
    #define SELF_PROVISION_PRODUCT PRODUCT_TDECK_V1
    #define SELF_PROVISION_MODEL   MODEL_D9
  #else
    #define SELF_PROVISION_PRODUCT PRODUCT_CARDPUTER_ADV
    #define SELF_PROVISION_MODEL   MODEL_EC
  #endif

  #ifndef CARDPUTER_ADV_DEFAULT_FREQ
    #define CARDPUTER_ADV_DEFAULT_FREQ 915000000UL
  #endif

  #ifndef CARDPUTER_ADV_DEFAULT_BW
    #define CARDPUTER_ADV_DEFAULT_BW 250000UL
  #endif

  #ifndef CARDPUTER_ADV_DEFAULT_SF
    #define CARDPUTER_ADV_DEFAULT_SF 11
  #endif

  #ifndef CARDPUTER_ADV_DEFAULT_CR
    #define CARDPUTER_ADV_DEFAULT_CR 5
  #endif

  #ifndef CARDPUTER_ADV_DEFAULT_TXP
    #define CARDPUTER_ADV_DEFAULT_TXP 14
  #endif

  uint8_t cardputer_adv_eeprom_read(uint8_t addr) {
    #if HAS_EEPROM
      return EEPROM.read(eeprom_addr(addr));
    #elif MCU_VARIANT == MCU_NRF52
      return eeprom_read(eeprom_addr(addr));
    #else
      return 0xFF;
    #endif
  }

  void cardputer_adv_eeprom_write_u32(uint8_t addr, uint32_t value) {
    eeprom_update(eeprom_addr(addr)+0x00, value>>24);
    eeprom_update(eeprom_addr(addr)+0x01, value>>16);
    eeprom_update(eeprom_addr(addr)+0x02, value>>8);
    eeprom_update(eeprom_addr(addr)+0x03, value);
  }

  void cardputer_adv_write_info() {
    ...
    uint32_t serial = (uint32_t)(mac & 0xFFFFFFFF);

    info[ADDR_PRODUCT] = SELF_PROVISION_PRODUCT;
    info[ADDR_MODEL] = SELF_PROVISION_MODEL;
    info[ADDR_HW_REV] = 0x01;
    info[ADDR_SERIAL+0x00] = serial>>24;
    info[ADDR_SERIAL+0x01] = serial>>16;
    info[ADDR_SERIAL+0x02] = serial>>8;
    info[ADDR_SERIAL+0x03] = serial;
    info[ADDR_MADE+0x00] = 0x00;
    info[ADDR_MADE+0x01] = 0x00;
    info[ADDR_MADE+0x02] = 0x00;
    info[ADDR_MADE+0x03] = 0x00;
  ...
  void cardputer_adv_write_default_conf() {
  ...
  void cardputer_adv_write_default_ui_conf() {
    if (cardputer_adv_eeprom_read(ADDR_CONF_DSET) != CONF_OK_BYTE) {
      eeprom_update(eeprom_addr(ADDR_CONF_DSET), CONF_OK_BYTE);
      eeprom_update(eeprom_addr(ADDR_CONF_DINT), 0xFF);
    }

    if (cardputer_adv_eeprom_read(ADDR_CONF_DIA) == 0xFF) {
  ...
  bool cardputer_adv_info_valid() {
    return eeprom_lock_set() &&
      cardputer_adv_eeprom_read(ADDR_PRODUCT) == SELF_PROVISION_PRODUCT &&
      cardputer_adv_eeprom_read(ADDR_MODEL) == SELF_PROVISION_MODEL &&
      eeprom_hwrev_valid() &&
  ...
  void cardputer_adv_self_provision() {
    ...
    cardputer_adv_write_default_ui_conf();
  }
#endif
